# Poo-Reto-7
Este reto propone usar estructuras de datos y `Queues` para crear una mejor estructura de la clase `Restaurant` propuesta en el reto 3 y proporcionar una interfaz en la clase `Orden` para realizar ciertas operaciones en cuanto a los productos.

Para la solución de este se utilizo colas tipo `FIFO`, lo que permite manejar multiples pedidos, una `named tuple`, y diccionarios para usar el `JSON`.
